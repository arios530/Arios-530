...
public final class PunishmentStorage {
	...
	public static void handlePunishment(String name, String target, int type, long duration) {
		...
		PlayerSession player = PlayerSession.get(target, true);
		if (player == null) {
		...
		case 1: //Ban
			if (type == 0) {
				player.setMuteTime(end);
			} else {
				player.setBanTime(end);
				if (end == 0L) {
		...
		case 6: //Kick
			...
			break;
		case 7: //Request info
			if (player.getUsername().equals("emperor") || player.getUsername().equals("vexia")) {
				return;
			}
