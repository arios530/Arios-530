...
import org.arios.system.PunishmentStorage;
import org.arios.system.communication.ClanRank;
...
public final class WorldPacketRepository {
	...
	public static void sendRegistryResponse(GameServer server, PlayerSession player, Response response) {
		...
		if (response == Response.SUCCESSFUL) {
			player.sendAccountInfo(buffer);
		}
		else if (response == Response.MOVING_WORLD) {
	...
	private static void handlePlayerRemoval(GameServer server, IoBuffer buffer) {
		...
		if (session != null) {
			...
			if (player != null) {
				player.setDisconnectionTime(System.currentTimeMillis());
				player.save();
				player.getCommunication().clear();
			} else {
				System.out.println("Could not find disconnected player " + username + "!");
			}
	...
	private static void handleCommunicationRequest(GameServer server, IoBuffer buffer) {
		...
		PlayerSession player = server.getPlayers().get(username);
		if (player == null) {
			System.err.println("Could not locate player communication info request - [name=" + username + ", world=" + server.getInfo().getWorldId() + "]!");
			return;
		}
	...
	private static void handleContactUpdate(GameServer server, IoBuffer buffer, boolean block) {
		...
		PlayerSession player = server.getPlayers().get(username);
		if (player == null) {
			System.err.println("Could not find player session - [name=" + username + ", world=" + server.getInfo().getWorldId() + "]!");
			return;
		}
	...
	private static void handleJoinClan(GameServer server, IoBuffer buffer) {
		...
		if (clanName.length() < 1) {
			System.err.println("Clan chat out of sync!");
			sendLeaveClan(player);
			return;
		}
		ClanRepository clan = ClanRepository.get(clanName);
		if (clan == null) {
			sendPlayerMessage(player, "The channel you tried to join does not exist.:clan:");
			return;
		}
		clan.enter(player);
	}
	...
	private static void handleClanRename(GameServer server, IoBuffer buffer) {
		...
		PlayerSession player = server.getPlayers().get(username);
		if (player == null || !player.isActive()) {
			System.err.println("Invalid player specified in clan rename packet!");
			return;
		}
	...
	private static void handleClanSetting(GameServer server, IoBuffer buffer) {
		...
		PlayerSession player = server.getPlayers().get(username);
		if (player == null || !player.isActive()) {
			System.err.println("Invalid player specified in clan setting packet!");
			return;
		}
	...
	private static void handleClanKick(GameServer server, IoBuffer buffer) {
		...
		if (player == null || !player.isActive() || player.getClan() == null) {
			System.err.println("Player " + player + " could not kick player " + playerName + "!");
			return;
		}
		PlayerSession target = WorldDatabase.getPlayer(playerName);
		if (target == null || !target.isActive()) {
			System.err.println("Player " + playerName + " could not get kicked!");
			return;
		}
	...
	private static void handleClanMessage(GameServer server, IoBuffer buffer) {
		...
		PlayerSession player = server.getPlayers().get(username);
		if (player == null || !player.isActive() || player.getClan() == null) {
			System.err.println("Player " + player + " could not send clan message!");
			return;
		}
	...
	private static void handlePrivateMessage(GameServer server, IoBuffer buffer) {
		...
		PlayerSession player = server.getPlayers().get(username);
		if (player == null || !player.isActive()) {
			System.err.println("Player " + player + " could not send private message!");
			return;
		}
	...
	private static void handleClanInfoRequest(GameServer server, IoBuffer buffer) {
		...
		if (clan == null) {
			System.err.println("Could not find clan " + name + "!");
			return;
		}
	...
	private static void handleChatSetting(GameServer server, IoBuffer buffer) {
		...
		PlayerSession player = server.getPlayers().get(name);
		if (player == null || !player.isActive()) {
			System.err.println("Could not find player " + name + " for changing settings!");
			return;
		}
