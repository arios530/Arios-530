...
import org.arios.net.producer.MSHSEventProducer;
...
public final class WorldCommunicator {
	...
	public static void register(final LoginParser parser) {
		...
		if (p != null && GameWorld.getTicks() - p.getTimeStamp() < 50 && p.getDetails().getRights() == Rights.REGULAR_PLAYER) {
			System.err.println("Duplicate login attempt [username=" + p.getDetails().getUsername() + "]!");
			parser.getDetails().getSession().write(Response.ALREADY_ONLINE, true);
			return;
		}
		parser.getDetails().setPassword(SystemManager.getEncryption().hashPassword(parser.getPassword()));
		parser.getDetails().setInfo(parser.getInfo());
		loginAttempts.put(parser.getDetails().getUsername(), parser);
		if (!parser.getDetails().isParsedSql()) {
			TaskExecutor.executeSQL(new Runnable() {
				@Override
				public void run() {
					parser.getDetails().parse();
					parser.getDetails().getSqlManager().parse();
					parser.getDetails().setInfo(parser.getInfo());
					if (GameWorld.getSettings().isBeta() && !SystemManager.getSystemConfig().getBetaUsers().contains(parser.getDetails().getUsername()) && !SystemManager.getSystemConfig().isStaff(parser.getDetails().getUsername()) && !parser.getDetails().getPortal().isDonator()) {
						parser.getDetails().getSession().write(Response.CLOSED_BETA, true);
						return;
					}
	...
	public static void connect() {
		try {
			...
			if (isLocallyHosted()) {
				...
				reactor = NioReactor.connect("127.0.0.1", 55555);
			} else {
				reactor = NioReactor.connect(GameWorld.getSettings().getMsAddress(), 55555);
			}
			reactor.start();
		} catch (Throwable e) {
			e.printStackTrace();
	...
	public static boolean isEnabled() {
		return false;//state == ManagementServerState.AVAILABLE;
	}
