...
public final class MSPacketRepository {
	...
	public static void sendPlayerRegistry(LoginParser parser) {
		...
		buffer.putString(parser.getPassword());
		...
		} else if (d.getSqlManager().getTable().getColumn("ironManMode").getValue() != null && !d.getSqlManager().getTable().getColumn("ironManMode").getValue().equals("NONE")) {
			icon = d.getSqlManager().getTable().getColumn("ironManMode").getValue().equals("STANDARD") ? IronmanMode.STANDARD.getIcon() : IronmanMode.ULTIMATE.getIcon();
		} else if (d.getPortal().isDonator()) {
			icon = d.getPortal().getDonatorType().getIcon().getIndexId();
		}
		buffer.put((byte) icon);
		WorldCommunicator.getSession().write(buffer);
	}
	...
	private static void handleRegistryResponse(IoBuffer buffer) {
		...
		if (parser != null) {
			...
			switch (response) {
			case SUCCESSFUL:
				details.setLastLogin(buffer.getLong());
				long muteTime = buffer.getLong();
				...
				details.setInfo(parser.getInfo());
				details.getPortal().setMuteTime(muteTime);
				Player player = new Player(details);
				parser.initialize(player);
	...
	private static void handleContactInformation(IoBuffer buffer) {
		...
		if (player == null || !player.isActive()) {
			// System.err.println("Inactive player contact info update [name=" +
		...
		if (buffer.get() == 1) {
			...
			sendJoinClan(player, GameWorld.getName().toLowerCase());
		}
	}
	...
	private static void handleContactUpdate(IoBuffer buffer) {
		...
		Player player = Repository.getPlayerByName(username);
		if (player == null || !player.isActive()) {
			// System.err.println("Inactive player contact update [name=" +
			// username + ", player=" + player + "]!");
			return;
		}
		switch (type) {
	...
	private static void handleLeaveClan(IoBuffer buffer) {
		...
		Player player = Repository.getPlayerByName(name);
		if (player == null || !player.isActive()) {
			// System.err.println("Player " + name + " was null (leave clan)!");
			return;
		}
		if (player.getCommunication().getClan() == null) {
			// System.err.println("Player " + name + " had no clan to leave!");
			return;
		}
	...
	private static void handleContactNotification(IoBuffer buffer) {
		...
		for (int i = 0; i < size; i++) {
			String username = buffer.getString();
			Player player = Repository.getPlayerByDisplay(username);
			if (player == null) {
				// System.err.println("Player " + username +
				// " was out of sync with MS!");
				continue;
			}
			Contact c = player.getCommunication().getContacts().get(name);
			if (c == null) {
				// System.err.println("Contact " + username +
				// " was out of sync with MS!");
				continue;
			}
	...
	private static void handlePunishmentUpdate(IoBuffer buffer) {
		...
		case 0:
			...
			player.getDetails().getPortal().setMuteTime(duration);
			break;
		case 1:
