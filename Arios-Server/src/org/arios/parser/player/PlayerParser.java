package org.arios.parser.player;
...
public final class PlayerParser {
	...
	public static void parse(Player player) {
		final File file = new File(ServerConstants.PLAYER_SAVE_PATH + player.getName() + ".arios");
		if (!file.exists()) {
			dump(player);
		}
	...
	public static void dump(Player player, String directory) {
		...
		buffer.flip();
		File file = new File(directory + "players/" + player.getName() + ".arios");
