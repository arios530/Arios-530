...
public abstract class Pathfinder {
	 ...
	 * @return The path.
	 */
	public abstract Path find(Location location, int size, Location end, int sizeX, int sizeY, int rotation, int type, int walkingFlag, boolean near);
	 ...
	 * @return The path.
	 */
	public static Path find(Location start, int moverSize, Node destination, boolean near, Pathfinder finder) {
		if (destination instanceof GameObject) {
			...
			if (type == 10 || type == 11 || type == 22) {
				...
				return finder.find(start, moverSize, destination.getLocation(), sizeX, sizeY, 0, 0, walkingFlag, near);
			}
			return finder.find(start, moverSize, destination.getLocation(), 0, 0, rotation, 1 + type, 0, near);
		}
		int size = 0;
		...
		return finder.find(start, moverSize, destination.getLocation(), size, size, 0, 0, 0, near);
	}
