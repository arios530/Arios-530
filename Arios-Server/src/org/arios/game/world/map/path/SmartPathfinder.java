...
public final class SmartPathfinder extends Pathfinder {
	...
	@Override
	public Path find(Location start, int moverSize, Location end, int sizeX, int sizeY, int rotation, int type, int walkingFlag, boolean near) {
		reset();
		Path path = new Path();
		foundPath = false;
		for (int x = 0; x < 104; x++) {
		...
		int z = start.getZ();
		Location location = Location.create((start.getRegionX() - 6) << 3, (start.getRegionY() - 6) << 3, z);
		...
		attempts = 0;
		for (int directionFlag = previousDirection = via[curX][curY]; curX != start.getSceneX() || curY != start.getSceneY(); directionFlag = via[curX][curY]) {
	 ...
	 */
	private void checkSingleTraversal(Location end, int sizeX, int sizeY, int type, int rotation, int walkingFlag, Location location) {
		int readPosition = 0;
		int z = location.getZ();
	 ...
	 * @param location The viewport location.
	 */
	private void checkDoubleTraversal(Location end, int sizeX, int sizeY, int type, int rotation, int walkingFlag, Location location) {
		int readPosition = 0;
		int z = location.getZ();
		while (writePathPosition != readPosition) {
			curX = queueX[readPosition];
	 ...
	 * @param location The viewport location.
	 */
	private void checkVariableTraversal(Location end, int size, int sizeX, int sizeY, int type, int rotation, int walkingFlag, Location location) {
		int readPosition = 0;
		int z = location.getZ();
