...
import org.arios.game.world.map.build.LandscapeParser;
import org.arios.game.world.map.build.MapscapeParser;
...
public class Region {
	...
	/**
	 * Checks if the region is inactive, if so it will start the inactivity
	 * flagging.
	...
	public boolean checkInactive() {
		...
		if (getId() == 512 && this instanceof DynamicRegion) {
			return false;
		}
	 ...
	 * Loads the flags for a region.
	 * @param r The region.
	 * @param storeObjects if all objects in this region should be stored
	 * (rather than just the ones with options).
	 */
	public static void load(Region r, boolean storeObjects) {
		try {
			...
			if (landscapeId > -1) {
				byte[] landscape = Cache.getIndexes()[5].getFileData(landscapeId, 0, XTEAKeyParser.getRegionXTEA(regionId));
				if (landscape == null || landscape.length < 4) {
					return;
				}
				r.hasFlags = true;
				try {
					LandscapeParser.parse(r, mapscapeData, ByteBuffer.wrap(landscape), storeObjects);
				} catch (Throwable t) {
					t.printStackTrace();
				}
			}
	 ...
	 * @deprecated This should not be used, instead use the
	 * {@link #flagInactive()}, {@link #flagActive()} & {@link #checkInactive()}
	 * methods to safely change the activity state.
	 */
	@Deprecated
	...
	/**
	 * Gets the real region id (this returns the copied region id for dynamic
	 * regions).
	 * @return The region id.
	 */
	...
	/**
	 * Gets the activityPulse.
	 * @return the activityPulse
	 */
	public Pulse getActivityPulse() {
