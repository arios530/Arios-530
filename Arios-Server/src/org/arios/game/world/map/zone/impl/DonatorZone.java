package org.arios.game.world.map.zone.impl;
...
public class DonatorZone extends MapZone {
	...
	public void configure() {
		...
		// addy
		addObject(new GameObject(2092, base.transform(17, 14, 0)));
		addObject(new GameObject(2093, base.transform(16, 13, 0)));
		addObject(new GameObject(2092, base.transform(17, 12, 0)));
		// mith
		...
		if (region != null) {
			...
			for (NPC n : region.getPlanes()[0].getNpcs()) {
				npc = NPC.create(n.getId(), base.transform(n.getLocation().getLocalX(), n.getLocation().getLocalY(), 0));
