...
import org.arios.game.system.script.context.ItemMessageInstruction;
import org.arios.game.system.script.context.NPCDialInstruction;
import org.arios.game.system.script.context.OptionDialInstruction;
import org.arios.game.system.script.context.PDialInstruction;
import org.arios.game.system.script.context.PlainMessageInstruction;
...
public final class DialogueInterpreter {
	...
	public Component sendPlaneMessage(final boolean hideContinue, String... messages) {
		...
		player.getPacketDispatch().sendInterfaceConfig(player.getInterfaceManager().getChatbox().getId(), messages.length, hideContinue);
		return player.getInterfaceManager().getChatbox();
	}
	...
	public Component sendItemMessage(int itemId, String... messages) {
		...
		player.getPacketDispatch().sendItemOnInterface(itemId, 1, 131, 0);
		player.getPacketDispatch().sendItemOnInterface(-1, 1, 131, 2);
		return player.getInterfaceManager().getChatbox();
	}
	...
	public Component sendItemMessage(final Item item, String... messages) {
		...
		player.getPacketDispatch().sendString(message, 131, 1);
		player.getPacketDispatch().sendItemOnInterface(item.getId(), item.getAmount(), 131, 0);
		return player.getInterfaceManager().getChatbox();
	}
