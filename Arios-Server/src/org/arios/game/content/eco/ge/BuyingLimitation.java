...
public final class BuyingLimitation {
	...
	public static int getMaximumBuy(int itemId, int playerUID) {
		...
		return ItemDefinition.forId(itemId).getConfiguration(ItemConfiguration.GE_LIMIT, 25000) - current;
	}
	...
	public static boolean isLimited(int itemId, int playerUID) {
		...
		int max = ItemDefinition.forId(itemId).getConfiguration(ItemConfiguration.GE_LIMIT, 25000);
		return current >= max;
	}
