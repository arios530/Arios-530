...
import org.arios.game.node.object.Constructed;
...
import org.arios.parser.object.DoorDefinitionsParser.Door;
...
public final class DoorActionHandler {
	...
	public static void handleDoor(final Player player, final GameObject object) {
		...
		if (object.getDefinition().hasAction("close")) {
			...
			Door d = DoorDefinitionsParser.forId(object.getId());
			if (d == null) {
		...
		if (second != null) {
			Door s = DoorDefinitionsParser.forId(second.getId());
	...
	public static Location getDestination(Entity entity, GameObject door) {
		/**
		 * if (DoorDefinitionsParser.forId(door.getId()) != null) {
		 * System.out.println
		 * ("door isnt found in parser?(door action handler)"); return null; }
		 */
	 ...
	 * closed again.
	 */
	private static void open(GameObject object, GameObject second, int replaceId, int secondReplaceId, boolean clip, int restoreTicks, boolean fence) {
