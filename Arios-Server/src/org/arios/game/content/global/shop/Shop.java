package org.arios.game.content.global.shop;
...
import org.arios.game.node.entity.player.link.spawn.PKPackage;
...
public class Shop {
	...
	public void open(final Player player) {
		if (pointShop && GameWorld.isEconomyWorld()) {
			return;
		}
	...
	public void buy(Player player, final int slot, int amount, int tabIndex) {
		...
		if (player.getRights().equals(Rights.ADMINISTRATOR) && !GameWorld.getSettings().isDevMode()) {
			return;
		}
		...
		if (currency == TOKKUL & !GameWorld.isEconomyWorld()) {
			player.sendMessage("You can't do that on this world.");
			return;
		}
	...
	public void sell(Player player, final int slot, int amount, int tabIndex) {
		...
		if (!GameWorld.isEconomyWorld()) {
			for (PKPackage pack : PKPackage.values()) {
				for (Item i : pack.getItems()) {
					if (i.getName().equalsIgnoreCase(item.getName())) {
						player.sendMessage("You can't sell spawnable items to the shops.");
						return;
					}
				}
			}
	...
	public int getSellingValue(Item item, Player player) {
		...
		if (getCurrency() == TOKKUL) {
			int tokkul = item.getDefinition().getConfiguration(ItemConfiguration.TOKKUL_PRICE, -1);
			if (tokkul > 0) {
				value = tokkul /= 10;
			}
		}
