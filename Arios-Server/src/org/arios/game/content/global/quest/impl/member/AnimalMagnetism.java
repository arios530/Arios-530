...
public final class AnimalMagnetism extends Quest {
	 ...
	 */
	public AnimalMagnetism(Player player) {
		super(player);
	}
	...
	private void drawRequirements() {
		...
		for (int i = 0; i < requirements.length; i++) {
			line((requirements[i] ? "<str>" : "<red>") + REQS[i], line++);
		}
	}

	@Override
	public boolean hasRequirements() {
		requirements[0] = player.getQuestRepository().isComplete(RestlessGhost.NAME);
		requirements[1] = player.getQuestRepository().isComplete(ErnestTheChicken.NAME);
	...
	public int[] getConfig() {
		if (getStage() >= 28 && getStage() != 100) {
			return new int[] { 939, 150 };
		}
		int val = getState() == QuestState.STARTED ? 10 : getState() == QuestState.COMPLETED ? 240 : 0;
