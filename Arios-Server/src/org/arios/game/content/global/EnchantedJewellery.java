	...
	private boolean teleport(final Player player, final int itemIndex, final Item item, final Location location) {
		...
		GameWorld.submit(new Pulse(4, player) {
			...
			public boolean pulse() {
				...
				if (!player.getDetails().getShop().hasPerk(Perks.CHARGE_BEFRIENDER)) {
					player.getPacketDispatch().sendMessage(isCrumble() && itemIndex == getIds().length - 1 || isLast(getItemIndex(item)) ? "You use your " + getNameType(item) + "'s last charge." : "Your " + getName(item) + " has " + Integer.parseInt(getCharges(item)) + " use" + (Integer.parseInt(getCharges(item)) > 1 ? "s" : "") + " left.");
				}
