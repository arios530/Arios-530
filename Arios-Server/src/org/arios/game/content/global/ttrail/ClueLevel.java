package org.arios.game.content.global.ttrail;
...
public enum ClueLevel {
	...
	public void open(Player player, Item casket) {
		...
		if (player.getTreasureTrailManager().isCompleted() || GameWorld.getSettings().isDevMode()) {
			...
			player.sendMessage("<col=FF0000>Completed clues: easy - " + player.getTreasureTrailManager().getCompletedClues(ClueLevel.EASY) + ", medium - " + player.getTreasureTrailManager().getCompletedClues(ClueLevel.MEDIUM) + ", hard - " + player.getTreasureTrailManager().getCompletedClues(ClueLevel.HARD) + "!");
			PacketRepository.send(ContainerPacket.class, new ContainerContext(player, 364, 1, 91, rewards.toArray(new Item[] {}), true));
			return;
		}
	...
	public List<Item> getLoot(Player player) {
		...
		int rand = RandomFunction.random(player.hasPerk(Perks.DETECTIVE) ? 2000 : 4000);
