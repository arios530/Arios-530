package org.arios.game.content.global.travel.canoe;
...
/**
 * Represents a class that i used to hold relative information of the states of
 * a canoe for a player.
 * @author 'Vexia
 */
public final class CanoeExtension {
	 ...
	 */
	private final Component shape = new Component(52);

	/**
	 * Represents the destination component.
	 */
	private final Component destination = new Component(53);
	 ...
	 */
	private final int[][] shapeConfigs = new int[][] { { 7, 7 }, { 32, 6 }, { 35, 3 }, { 29, 13 } };

	/**
	 * Represents the boat childs (indexes)
	 */
	private final int boatChilds[] = new int[] { 11, 13, 15, 38, 31 };

	/**
	 * Represents the location childs (indexes)
	 */
	private final int locationChilds[] = new int[] { 50, 47, 44, 36 };

	/**
	 * Represents the distances to travel(indexes)
	 */
	private final int distances[] = new int[] { 1, 2, 4, 4 };
	...
	public final void shape(final GameObject object) {
		...
		for (int i = 0; i < Canoe.values().length; i++) {
			...
			public boolean pulse() {
				...
				case 17:
					...
					if (station == CanoeStation.WILDERNESS) {
						...
						player.getInterfaceManager().openOverlay(new Component(380));
					}
					setCanoe(null);
					setStation(null);
					setStage(0);
