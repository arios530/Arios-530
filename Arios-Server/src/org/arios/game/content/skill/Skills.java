...
public final class Skills {
	 ...
	 */
	public static final double EXPERIENCE_MULTIPLIER = 10;
	...
	public void addExperience(int slot, double experience, boolean playerMod) {
		...
		if (entity instanceof Player) {
			...
			if (p.getRights() == Rights.ADMINISTRATOR) {
				if (p.getAttribute("no_experience_gain_admin", false)) {
					return;
				}
			}
			...
			if (TutorialSession.getExtension(p).getStage() < TutorialSession.MAX_STAGE) {
				...
				if (playerMod && p.getExperienceMod() != 0.0) {
					if (!SystemManager.getSystemConfig().getConfig("dxp", false)) {
						...
						mod += (p.getExperienceMod() / 2) * 10;
					}
				}
			}
			final AssistSession assist = p.getExtension(AssistSession.class);
			if (assist != null) {
				if (assist.translateExperience(p, slot, experience, mod)) {
					return;
				}
			}
		}
		boolean send200m = false;
		if(this.experience[slot] != 200000000 && this.hasLevel(slot, 99)){
			send200m = true;
		}
		...
		if(send200m && this.experience[slot] >= 200000000){
