...
public final class FarmingEquipment implements SavingModule {
	...
	public void open(final Player player) {
		final Item[] first = Arrays.copyOfRange(ITEMS, 0, 6);
		final Item[] second = Arrays.copyOfRange(ITEMS, 6, 9);
		player.getInterfaceManager().open(STORE_COMPONENT);
		player.getInterfaceManager().openSingleTab(TAB_COMPONENT);
		PacketRepository.send(ContainerPacket.class, new ContainerContext(player, 125, 68, 91, new Container(21, first), true));
		PacketRepository.send(ContainerPacket.class, new ContainerContext(player, 125, 69, 91, new Container(21, second), true));
		PacketRepository.send(ContainerPacket.class, new ContainerContext(player, 126, 0, 91, new Container(21, first), true));
		PacketRepository.send(ContainerPacket.class, new ContainerContext(player, 126, 1, 91, new Container(21, second), true));
		update(player);
	}
