...
import org.arios.game.content.skill.member.farming.patch.PatchProtection;
import org.arios.game.content.skill.member.farming.wrapper.PatchCycle;
...
public class FarmingNode {
	...
	public int getHarvestAmount(final PatchCycle cycle) {
		...
		int mod = 0;
		if (cycle.getPlayer().getDetails().getShop().hasPerk(Perks.GREEN_THUMB)) {
			mod += RandomFunction.random(3, 15);
		}
