...
import org.arios.game.content.skill.member.hunter.NetTrapSetting.NetTrap;
...
public enum Traps {
	...
	public static Traps forNode(Node node) {
		for (Traps trap : Traps.values()) {
			...
			if (trap == NET_TRAP) {
				for (NetTrap net : NetTrap.values()) {
