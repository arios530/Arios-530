...
public abstract class Familiar extends NPC implements Plugin<Object> {
	...
	public void handleTickActions() {
		...
		if (!isHidden() && !getProperties().getCombatPulse().isAttacking() && (combat.isAttacking() || owner.inCombat())) {
			...
			if (combat.getVictim() != this && victim != null && getProperties().isMultiZone() && owner.getProperties().isMultiZone() && isCombatFamiliar() && !isBurdenBeast()) {// Splinter
	...
	public boolean isAttackable(Entity entity, CombatStyle style) {
		if (entity == owner) {
			owner.getPacketDispatch().sendMessage("You can't attack your own familiar!");
			return false;
		}
	...
	public boolean call() {
		...
		setHidden(getZoneMonitor().isRestricted(ZoneRestriction.FOLLOWERS) && !owner.getLocks().isLocked("enable_summoning"));
