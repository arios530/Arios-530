...
public final class AntiMacroHandler implements SavingModule {
	...
	public void pulse() {
		...
		if (!player.getLocks().isInteractionLocked() && !player.getLocks().isTeleportLocked() && !player.getLocks().isMovementLocked()) {
			for (int i = 0; i < monitors.length; i++) {
				FIRE_RATIO = (player.getDetails().getShop().hasPerk(Perks.EVENT_AVOIDER) ? 400 : 250);
	...
	public boolean fireEvent(String name, Object... args) {
		if (hasEvent() || player.getZoneMonitor().isRestricted(ZoneRestriction.RANDOM_EVENTS) || !GameWorld.isEconomyWorld()) {
			return false;
		}
	...
	public boolean fireEvent(int skillId, Object... args) {
		if (hasEvent() || EVENTS.isEmpty() || player.getZoneMonitor().isRestricted(ZoneRestriction.RANDOM_EVENTS) || !GameWorld.isEconomyWorld()) {
			return false;
		}
