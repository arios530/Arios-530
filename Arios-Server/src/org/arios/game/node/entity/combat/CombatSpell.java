package org.arios.game.node.entity.combat;
...
public abstract class CombatSpell extends MagicSpell {
	 ...
	 */
	public CombatSpell(SpellType type, SpellBook book, int level, double baseExperience, int castAudio, int impactAudio, Animation animation, Graphics startGraphic, Projectile projectile, Graphics endGraphic, Item... runes) {
	...
	public void visualize(Entity entity, Node target) {
		...
		if (entity.getProperties().getAutocastSpell() == this && (entity instanceof Player || animation == null)) {
			...
			if (entity instanceof NPC) {
				NPC n = entity.asNpc();
				if (n.getDefinition().getCombatAnimations()[1] != null && n.getDefinition().getCombatAnimations()[1].getId() != -1) {
					entity.animate(n.getDefinition().getCombatAnimations()[1]);
				} else {
