...
public abstract class CombatSwingHandler {
	...
	public boolean isAccurateImpact(Entity entity, Entity victim, CombatStyle style, double accuracyMod, double defenceMod) {
		...
		if (style != null) {
			...
			} else if (victim instanceof Player && entity instanceof Familiar && ((Player) victim).getPrayer().get(PrayerType.PROTECT_FROM_SUMMONING)) {
				mod = 0;
			}
		}
	...
	protected int getFormatedHit(Entity entity, Entity victim, BattleState state, int hit) {
		...
		if (entity instanceof Familiar && victim instanceof Player) {
