package org.arios.game.node.entity.player.link.request.trade;
...
public final class TradeContainer extends Container {
	...
	private boolean tradeable(final Item item) {
		...
		if (item.getId() == ShieldofArrav.BLACKARM_CERTIFICATE.getId() || item.getId() == ShieldofArrav.PHOENIX_CERTIFICATE.getId() || item.getId() == ShieldofArrav.KEY.getId()) {
			return true;
		}
