...
public class AchievementDiary implements SavingModule {
	...
	public void open(Player player) {
		clear(player);
		sendString(player, "<red>Achievement Diary - " + type.getName() + "<br><br>", 2);
		StringBuilder builder = new StringBuilder((isComplete() ? GREEN : hasStarted() ? YELLOW : "<red>") + type.getName() + " Area Tasks<br><br>");
		boolean complete;
		for (int level = 0; level < type.getAchievements().length; level++) {
			builder.append("<br><br>" + getStatus(level) + getLevel(level) + "");
			for (int i = 0; i < type.getAchievements(level).length; i++) {
				complete = isComplete(level, i);
				builder.append("<br><br>" + (complete ? "<str>" : "") + (complete ? type.getAchievements(level)[i].replace("<br><br>", "<br><br><str>") : type.getAchievements(level)[i]));
			}
		}
		...
		sendString(player, "DSDSDSDS", 100);
	}
	...
	public void drawStatus(Player player) {
		if (hasStarted()) {
			...
			for (int i = 1; i <= 3; i++) {
				player.getPacketDispatch().sendString((isComplete(i-1) ? GREEN : hasStarted(i-1) ? YELLOW : RED) + getLevel(i-1), 259, type.getChild() + i);
			}
		}
	}
	...
	public void updateTask(Player player, int level, int index, boolean complete) {
		...
		if (!complete) {
			...
			player.sendMessages("Well done! You have complete " + (level == 0 ? "an easy" : level == 1 ? "a medium" : "a hard") + " task in the " + type.getName() + " area. Your Achievement", "Diary has been updated.");
		}
		if (isComplete(level)) {
			player.sendMessages("You have completed all of the " + getLevel(level).toLowerCase() + " tasks in the " + type.getName() + " area.", "Speak to " + NPCDefinition.forId(type.getNpc(level)).getName() + " to claim your reward.");
		}
		drawStatus(player);
	}
