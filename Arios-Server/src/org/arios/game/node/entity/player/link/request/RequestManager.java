package org.arios.game.node.entity.player.link.request;
...
public final class RequestManager {
	...
	private boolean canRequest(RequestType type, Player target) {
		...
		if (!GameWorld.getSettings().isDevMode() && target.getDetails().getRights() == Rights.ADMINISTRATOR && !target.getName(true).equals("vexia") && !target.getName(true).equals("splinter")) {
			player.sendMessage("You can't do that with an Arios moderator.");
			return false;
		}
		if (!target.getLocation().withinDistance(player.getLocation(), 15)) {
			player.getPacketDispatch().sendMessage("Unable to find " + target.getName(true) + ".");
			return false;
		}
