...
import org.arios.game.content.global.GodBook;
...
public final class GlobalData implements SavingModule {
	 ...
	 * The daily tasks
	 */
	private int[] dailyTasks = new int[35];
	...
	/**
     * The player's saved zoom amount, 600 is default for the client.
     */
    private int clientZoom = 600;
	...
	public void save(ByteBuffer buffer) {
		...
		SavedData.save(buffer, clientZoom, 46);
		// SavedData.save(buffer, dailyTasks, 46);
		buffer.put((byte) 0);
	}
	...
	public void parse(ByteBuffer buffer) {
		...
		while ((opcode = buffer.get()) != 0) {
			...
			case 45:
				...
				break;
			case 46:
				clientZoom = buffer.getInt();
				break;
			/*
			 * case 46: for (int i = 0; i < dailyTasks.length; i++) {
			 * dailyTasks[i] = buffer.getInt(); } break;
			 */
			}
		}
	}
	...
	/**
	 * Sets the dailyTasks
	 * @param dailyTasks the dailyTasks to set
	 */
