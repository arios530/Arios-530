 ...
 */
public enum PKTitle {
	NOOB(0), PEASANT(5), SKIRMISHER(20), KNIGHT(50), WARRIOR(80), BOUNTY_HUNTER(100), HERO(150), SKULLCRACKER(250), DESTROYER(400), DOMINATOR(600), REAPER(800, "DF7401"), ASSASSIN(1000, "21610B"), MASTER(1500, "0431B4"), GODLY(2000, "FF0000");
	...
	public static void checkTitle(Player player) {
		...
		if (title.ordinal() > player.getSavedData().getSpawnData().getTitle().ordinal() && player.getSavedData().getSpawnData().getKills() <= title.getKills()) {
			...
			if (!title.getTitleColor().equals("874b93")) {
				Repository.sendNews(player.getUsername() + " has just unlocked the title: " + title.getName() + "!");
			}
