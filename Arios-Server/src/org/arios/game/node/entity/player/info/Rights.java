...
public enum Rights {
	...
	public static int getChatIcon(Player player) {
		...
		if (c != Rights.REGULAR_PLAYER) {
			return c.toInteger();
		}
		if (player.getIronmanManager().isIronman()) {
			return player.getIronmanManager().getMode().getIcon();
		}
		if (player.getDetails().getPortal().isDonator()) {
			return player.getDetails().getPortal().getDonatorType().getIcon().getIndexId();
		}
		return 0;
	}
	 ...
	 */
	public static Rights forId(final int id) {
		return values()[id];
	}
