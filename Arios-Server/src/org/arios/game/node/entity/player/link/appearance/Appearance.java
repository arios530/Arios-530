package org.arios.game.node.entity.player.link.appearance;
...
public final class Appearance implements SavingModule {
	...
	public void transformNPC(int id) {
		...
		if (id == -1) {
			...
			if (player.getEquipment().get(3) != null) {
				anims = player.getEquipment().get(3).getDefinition().getConfiguration(ItemConfiguration.ATTACK_ANIMS, anims);
			}
			...
			if (def.getCombatAnimations()[0] != null) {
				player.getProperties().setAttackAnimation(def.getCombatAnimations()[0]);
			}
			if (def.getCombatAnimations()[3] != null) {
				player.getProperties().setDefenceAnimation(def.getCombatAnimations()[3]);
			}
			if (def.getCombatAnimations()[4] != null) {
				player.getProperties().setDeathAnimation(def.getCombatAnimations()[4]);
			}
		}
		sync();
	}
	...
	public void setAnimations() {
		if (npcId == -1) {
			...
			if (weapon == null || weapon.getDefinition().getConfiguration(ItemConfiguration.WALK_ANIM) == null) {
				setDefaultAnimations();
			} else {
				ItemDefinition def = weapon.getDefinition();
				setStandAnimation(def.getConfiguration(ItemConfiguration.STAND_ANIM, 0x328));
				setStandTurnAnimation(def.getConfiguration(ItemConfiguration.STAND_TURN_ANIM, 0x337));
				setWalkAnimation(def.getConfiguration(ItemConfiguration.WALK_ANIM, 0x333));
				setRunAnimation(def.getConfiguration(ItemConfiguration.RUN_ANIM, 0x338));
				setTurn180(def.getConfiguration(ItemConfiguration.TURN180_ANIM, 0x334));
				setTurn90cw(def.getConfiguration(ItemConfiguration.TURN90CW_ANIM, 0x335));
				setTurn90ccw(def.getConfiguration(ItemConfiguration.TURN90CCW_ANIM, 0x336));
	...
	public void setAnimations(Animation anim) {
		int id = anim.getId();
		setStandAnimation(id);
		setStandTurnAnimation(id);
		setWalkAnimation(id);
		setRunAnimation(id);
		setTurn180(id);
		setTurn90cw(id);
	...
	public void prepareBodyData(Player player) {
		...
		if (hat != null && hat.getDefinition().getConfiguration(ItemConfiguration.REMOVE_BEARD, false)) {
			clearBodyPart(11);
		} else {
	 ...
	 */
	public void changeChender(Gender gender) {
		setGender(gender);
		sync();
	}
