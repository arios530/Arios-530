...
import org.arios.game.node.entity.player.link.emote.EmoteManager;
		...
		if (!getZoneMonitor().handleDeath(killer) && (!getProperties().isSafeZone() && getZoneMonitor().getType() != ZoneType.SAFE.getId()) && getDetails().getRights() != Rights.ADMINISTRATOR) {
			...
			for (Item item : c[1].toArray()) {
				if (item != null) {
					...
					items.add(ground);
					/*
					 * if killer is an ironman then we set the dropper to the
					 * player who was killed, so the ironman can't pick it up
					 */
