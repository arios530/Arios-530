...
import org.arios.game.node.entity.combat.CombatPulse;
import org.arios.game.node.entity.combat.CombatSpell;
import org.arios.game.node.entity.combat.equipment.ArmourSet;
...
public final class Properties {
	 ...
	 */
	private Animation attackAnimation = new Animation(422, Priority.HIGH);
	 ...
	 * The death animation.
	 */
	private Animation deathAnimation = new Animation(9055, Priority.HIGH);
	...
	public void updateDefenceAnimation() {
		if (entity instanceof NPC) {
			Animation animation = ((NPC) entity).getDefinition().getCombatAnimations()[3];
			if (animation != null) {
		...
		if (item != null) {
			defenceAnimation = item.getDefinition().getConfiguration(ItemConfiguration.DEFENCE_ANIMATION, Animation.create(1156));
		} else if ((item = c.get(EquipmentContainer.SLOT_WEAPON)) != null) {
			defenceAnimation = item.getDefinition().getConfiguration(ItemConfiguration.DEFENCE_ANIMATION, Animation.create(424));
		} else {
