package org.arios.game.system.communication;
...
public final class ClanRepository {
	 ...
	 */
	private ClanRank lootRequirement = ClanRank.ARIOS_MOD;
	...
	public boolean enter(Player player) {
		if (!owner.equals("arios") && players.size() >= MAX_MEMBERS) {
		...
		if (!player.getName().equals(owner) && player.getDetails().getRights() != Rights.ADMINISTRATOR) {
			...
			Player o = Repository.getPlayerByDisplay(owner);
			if (o != null) {
	...
	public void updateSettings(Player player) {
		...
		// << 18 | (isLootshare() ? 0 : 1));
		player.getPacketDispatch().sendString(joinRequirement.getInfo(), 590, 31);
		player.getPacketDispatch().sendString(messageRequirement.getInfo(), 590, 32);
		player.getPacketDispatch().sendString(kickRequirement.getInfo(), 590, 33);
		player.getPacketDispatch().sendString(lootRequirement.getInfo(), 590, 34);
	}
	...
	public static ClanRepository get(String owner, boolean create) {
		...
		if (details == null) {
			File file = new File(ServerConstants.PLAYER_DETAILS_PATH + owner + ".arios");
			if (!file.exists()) {
				return null;
			}
			details = Repository.getPlayerDetailFile(owner);
		}
