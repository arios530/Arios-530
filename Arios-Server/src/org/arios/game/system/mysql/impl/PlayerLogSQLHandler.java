...
import org.arios.game.system.monitor.MessageLog;
...
public class PlayerLogSQLHandler extends SQLEntryHandler<PlayerMonitor> {
	...
	public PlayerLogSQLHandler(PlayerMonitor entry, String playerName) {
		super(entry, "player_logs", "username", playerName);
	}
	...
	public void save() throws SQLException {
		...
		if (connection.prepareStatement("SELECT * FROM player_logs WHERE username='" + value + "' LIMIT 1").executeQuery().next()) {
			String b = "SET ";
			int size = 0;
			...
			PreparedStatement statement = connection.prepareStatement("UPDATE player_logs " + b + " WHERE username='" + value + "'");
			...
			PreparedStatement statement = connection.prepareStatement("INSERT INTO player_logs (username,public_chat,private_chat,clan_chat,address_log,command_log,trade_log,ge_log,duel_log,duplication_log) VALUES(?,?,?,?,?,?,?,?,?,?)");
