...
public final class ChaosTunnelZone extends MapZone implements Plugin<Object> {
	...
	private void commenceBorkBattle(Player player) {
		...
		boolean setTimer = !player.hasPerk(Perks.BORKAHOLIC);
		if (!setTimer) {
			int cap = (int) Perks.BORKAHOLIC.getData()[0];
			byte borkKills = player.getDetails().getShop().get("borkKills", (byte) 0);
			borkKills += 1;
			int left = cap - borkKills;
			player.sendMessage("You have " + (left <= 0 ? "no more " : left) + " bork kill" + (left == 1 ? "" : "s") + " left.");
			player.getDetails().getShop().getMapping().put("borkKills", (byte) borkKills);
			if (left <= 0) {
				player.getDetails().getShop().getMapping().put("borkKills", (byte) 0);
				setTimer = true;
				player.sendMessage("You cannot use bork for another 24 hours after this fight.");
			}
		}
		if (setTimer) {
