...
import org.arios.game.content.skill.member.slayer.Master;
...
public final class SlayerMasterDialogue extends DialoguePlugin {
	...
	public boolean handle(int interfaceId, int buttonId) {
		...
		case 1:
			...
			break;
		case 20:
			interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, "I have a wide selection of Slayer equipment; take a look!");
			stage = 21;
			break;
		case 21:
			end();
		...
		case 30:
			...
			break;
		case 10:
			interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, "I'm one of the elite Slayer Masters.");
			stage = 11;
			break;
		case 11:
		...
		case 12:
			...
			break;
		case 100:
			interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, "Oh dear, what do they teach you in school?");
			stage = 101;
			break;
		case 101:
			player("Well....er...");
			...
			break;
		case 102:
			interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, "I suppose I'll have to educate you, then. A Slayer is", "someone who is trained to fight specific creatures. They", "know those creatures' every weakenss and strength. As", "you can guess, it makes killing those creatures a lot");
			stage = 103;
			break;
		case 103:
			interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, "easier.");
			stage = 104;
			break;
		case 104:
		...
		case 105:
			...
			break;
		case 500:
			interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, "Hmmm, well, I'm not so sure...");
			stage = 501;
			break;
		case 501:
		...
		case 502:
			if (!master.hasRequirment(player)) {
				interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, "Sorry, but you're not strong enough to be taught by", "me.");
				stage = 99;
				break;
			}
			interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, "Oh, okay then; you twisted my arm. You'll have to", "train against specific groups of creatures.");
			stage = 503;
			break;
		case 503:
		...
		case 504:
			if (player.getInventory().freeSlots() != 0) {
				...
				interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, "We'll start you off hunting " + player.getSlayer().getTaskName() + "'s, you'll need to", "kill " + player.getSlayer().getAmount() + " of them.");
				stage = 510;
		...
		case 99:
			...
			break;
		case 510:
			interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, "You'll also need this enchanted gem. It allows Slayer", "Masters like myself to contact you and update you on", "your progress. Don't worry if you lose it; you can buy", "another from any Slayer Master.");
			stage = 511;
			break;
		case 511:
			player("Okay, great!");
			...
			break;
		case 1000:
			interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, "That's what you think..");
			stage = 99;
			break;
		case 2000:
			interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, "I am one of the greatest Slayer masters!");
			stage = 99;
			break;
		case 700:
			switch (buttonId) {
		...
		case 701:
			...
			if (!player.getSlayer().hasTask()) {
				player.getSlayer().generate(master);
				interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, "Excellent, you're doing great. Your new task is to kill", "" + player.getSlayer().getAmount() + " " + player.getSlayer().getTaskName() + "'s.");
				stage = 844;
				break;
			}
			if (Master.hasSameTask(master, player.getSlayer().getMaster(), player) && !player.getDetails().getShop().hasPerk(Perks.SLAYER_BETRAYER)) {
				interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, "You're still hunting something. Come back when you've", "finished your task.");
				stage = 99;
			} else {
				player.getSlayer().generate(master);
				interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, "Excellent, you're doing great. Your new task is to kill", "" + player.getSlayer().getAmount() + " " + player.getSlayer().getTaskName() + "'s.");
		...
		case 854:
			switch (buttonId) {
			case 1:
				interpreter.sendDialogues(master.getNpc(), FacialExpression.NORMAL, player.getSlayer().getTask().getTip());
				stage = 860;
				break;
			case 2:
				player("Okay, great!");
	...
	@Override
	public int[] getIds() {
		return new int[] { 70, 1598, 1596, 1597, 1599 };
	}
