...
public final class EnchantedGemDialogue extends DialoguePlugin {
	...
	public boolean open(Object... args) {
		...
		interpreter.sendDialogues(player.getSlayer().getMaster().getNpc(), FacialExpression.NORMAL, "Hello there " + player.getUsername() + ", what can I help you with?");
		stage = 0;
		return true;
	}
	...
	public boolean handle(int interfaceId, int buttonId) {
		...
		case 10:
			if (!player.getSlayer().hasTask()) {
				interpreter.sendDialogues(player.getSlayer().getMaster().getNpc(), FacialExpression.NORMAL, "You need something new to hunt. Come and see me", "When you can and I'll give you a new task.");
				stage = 11;
				break;
			}
			interpreter.sendDialogues(player.getSlayer().getMaster().getNpc(), FacialExpression.NORMAL, "You're currently assigned to kill " + NPCDefinition.forId((player.getSlayer().getTask().getNpcs()[0])).getName().toLowerCase() + "'s;", "only " + player.getSlayer().getAmount() + " more to go.");
			stage = 11;
			break;
		case 11:
		...
		case 12:
			switch (buttonId) {
			case 1:
				interpreter.sendDialogues(player.getSlayer().getMaster().getNpc(), FacialExpression.NORMAL, "My name's " + NPCDefinition.forId(player.getSlayer().getMaster().getNpc()).getName() + ", I'm the Slayer Master best able", "to train you.");
				stage = 21;
				break;
			case 2:
				interpreter.sendDialogues(player.getSlayer().getMaster().getNpc(), FacialExpression.NORMAL, "You'll find me in " + masterLocation() + ", I'll be here when you need a", "new task.");
				stage = 31;
				break;
			case 3:
				if (!player.getSlayer().hasTask()) {
					interpreter.sendDialogues(player.getSlayer().getMaster().getNpc(), FacialExpression.NORMAL, "You need something new to hunt.");
					stage = 99;
					break;
				}
				interpreter.sendDialogues(player.getSlayer().getMaster().getNpc(), FacialExpression.NORMAL, player.getSlayer().getTask().getTip());
				stage = 401;
				break;
			case 4:
	...
	public String masterLocation() {
		if (player.getSlayer().getMaster().getNpc() == org.arios.game.content.skill.member.slayer.Master.MAZCHNA.getNpc())
			return "Canifis";
		else if (player.getSlayer().getMaster().getNpc() == org.arios.game.content.skill.member.slayer.Master.TURAEL.getNpc())
			return "Taverley";
		else if (player.getSlayer().getMaster().getNpc() == org.arios.game.content.skill.member.slayer.Master.CHAELDAR.getNpc())
			return "Zanaris";
		else if (player.getSlayer().getMaster().getNpc() == org.arios.game.content.skill.member.slayer.Master.VANNAKA.getNpc())
			return "Edgeville dungeon";
		else if (player.getSlayer().getMaster().getNpc() == org.arios.game.content.skill.member.slayer.Master.DURADEL.getNpc())
