...
public final class FishingExplosivePlugin extends OptionHandler {
	...
	public static final class FishingExplosiveHandler extends UseWithHandler {
		 ...
		 */
		private static final int MOGRE_ID = 2801;
		...
		public boolean handle(final NodeUsageEvent event) {
			...
			if (!player.getSlayer().hasTask() || player.getSlayer().getTask() != Tasks.MOGRES.getTask() || player.getAttribute("hasMogre", false)) {
			...
			GameWorld.submit(new Pulse(delay, player) {
				...
				public boolean pulse() {
					...
					player.getPacketDispatch().sendPositionedGraphics(SPLASH_GRAPHIC, event.getUsedWith().getLocation());
