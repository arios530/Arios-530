...
public final class SummoningTrainingRoom extends OptionHandler {
	...
	public boolean handle(final Player player, Node node, String option) {
		...
		int questVal = quest.getStage() == 0 ? 0 : quest.getStage() > 0 && quest.getStage() < 100 ? 5 : 28893;
		switch (option) {
		case "close":
		...
		case "climb-up":
			...
			case 50:
				...
				if (quest.getStage() == 50 && (!player.getInventory().containsItem(WOLF_POUCH) || !player.getInventory().containsItem(HOWL_SCROLL))) {
			...
			return true;
		case "search":
			if (quest.getStage() == 30 && !player.getAttribute("searched-body", false)) {
