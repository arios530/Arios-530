...
public final class BarbarianOutpostCourse extends AgilityCourse {
	...
	private void handleLogBalance(final Player player, final GameObject object) {
		...
		AgilityHandler.walk(player, failed ? -1 : 1, Location.create(2551, 3546, 0), end, Animation.create(762), failed ? 0.0 : 13.5, failed ? null : "...You make it safely to the other side.");
		if (failed) {
			AgilityHandler.walk(player, -1, player.getLocation(), Location.create(2545, 3546, 0), Animation.create(762), 0.0, null);
	...
	private void handleLedgeBalance(final Player player, final GameObject object) {
		...
		AgilityHandler.walk(player, failed ? -1 : 3, Location.create(2536, 3547, 1), end, Animation.create(756), failed ? 0.0 : 22, failed ? null : "You skillfully edge across the gap.");
		...
		return new Pulse(1, player) {
			...
			public boolean pulse() {
				...
				case 7:
					...
					AgilityHandler.walk(player, -1, Location.create(2545, 3545, 0), Location.create(2545, 3543, 0), Animation.create(772), 0.0, null);
					break;
				case 11:
