...
public final class BrimhavenCourse extends OptionHandler {
	...
	private static void handleBalancingRope(final Player player, GameObject object) {
		...
		AgilityHandler.walk(player, -1, player.getLocation(), player.getLocation().transform(dir.getStepX() * 7, dir.getStepY() * 7, 0), Animation.create(762), getExp(player, 10.0), null);
	}
		...
		GameWorld.submit(new Pulse(2, player) {
			...
			public boolean pulse() {
				if (++count == 1) {
					...
					setDelay(7);
					AgilityHandler.walk(player, -1, player.getLocation().transform(dir), player.getLocation().transform(dir.getStepX() * 7, dir.getStepY() * 7, 0), Animation.create(744), 0.0, null);
	...
	private static void handleBalancingLedge(final Player player, GameObject object) {
		...
		AgilityHandler.walk(player, -1, player.getLocation(), end, Animation.create(756 - (diff << 1)), getExp(player, xp), null);
	}
	...
	private static void handlePlankObstacle(final Player player, GameObject object) {
		...
		if (object.getCharge() == 500) { // Plank is broken
			player.lock(7);
			AgilityHandler.walk(player, -1, start, start.transform(dir.getStepX() * 2, dir.getStepY() * 2, 0), ForceMovement.WALK_ANIMATION, 0.0, null);
	...
	private static void handleLogBalance(final Player player, GameObject object) {
		...
		AgilityHandler.walk(player, -1, player.getLocation(), player.getLocation().transform(dir.getStepX() * 7, dir.getStepY() * 7, 0), Animation.create(762), getExp(player, 12.0), null);
	}
