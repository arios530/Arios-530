...
import org.arios.game.content.skill.member.agility.AgilityCourse;
...
public final class AgilityPyramidCourse extends AgilityCourse {
	...
	private void handleLedge(final Player player, final GameObject object) {
		...
		if (fail) {
			player.lock(4);
			AgilityHandler.walk(player, -1, player.getLocation(), end, Animation.create(756 - (diff << 1)), 0.0, "You slip and fall to the level below.");
		...
		AgilityHandler.walk(player, 3, player.getLocation(), end, Animation.create(756 - (diff << 1)), 52, "You skillfully edge across the gap.");
	}
	...
	private void handlePlank(final Player player, final GameObject object) {
		...
		AgilityHandler.walk(player, fail ? -1 : 1, player.getLocation(), end, Animation.create(762), fail ? 0.0 : 56.4, fail ? null : "You walk carefully across the slippery plank...");
		if (fail) {
	...
	private void handleJumpGap(Player player, GameObject object) {
		...
		player.lock(4);
		AgilityHandler.forceWalk(player, 2, player.getLocation(), player.getLocation().transform(dir, 3), Animation.create(3067), 20, 56.4, null);
	}
	...
	private void handleGapCross(final Player player, final GameObject object) {
		...
		final Animation animation = Animation.create(3060 - mod);
		if (fail) {
			Location dest = object.getLocation().transform(dir, 1);
