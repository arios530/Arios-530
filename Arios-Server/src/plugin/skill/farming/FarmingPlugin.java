...
import org.arios.game.content.skill.member.farming.FarmingConstant;
import org.arios.game.content.skill.member.farming.FarmingNode;
import org.arios.game.content.skill.member.farming.FarmingPatch;
import org.arios.game.content.skill.member.farming.pot.Saplings;
import org.arios.game.content.skill.member.farming.tool.PatchTool;
	...
	public final class FarmingEquipmentStore extends ComponentPlugin {
		...
		public boolean handle(final Player player, Component component, int opcode, int button, final int slot, int itemId) {
			...
			case 125:
				switch (button) {
				case 68:
					player.getFarmingManager().getEquipment().remove(player, slot, 1);
					break;
				case 69:
					...
					if (amount == -1) {
						player.setAttribute("runscript", new RunScript() {
							...
							public boolean handle() {
								player.getFarmingManager().getEquipment().remove(player, 6 + slot, (int) getValue());
								return true;
							}
						});
			...
			case 126:
				switch (button) {
				case 0:
					player.getFarmingManager().getEquipment().store(player, slot, 1);
					break;
				case 1:
					final int amount = getAmount(player, slot + 6, opcode);
					if (amount == -1) {
						player.setAttribute("runscript", new RunScript() {
							@Override
							public boolean handle() {
								player.getFarmingManager().getEquipment().store(player, 6 + slot, (int) getValue());
								return true;
							}
						});
						player.getDialogueInterpreter().sendInput(false, "Enter amount:");
