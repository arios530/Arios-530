...
public final class WormbrainNPC extends AbstractNPC {
	...
	public void finalizeDeath(final Entity killer) {
		...
		if (killer instanceof Player) {
			if (((Player) killer).getQuestRepository().getQuest("Dragon Slayer").getStage() == 20 && !((Player) killer).getInventory().containsItem(DragonSlayer.WORMBRAIN_PIECE) && !((Player) killer).getBank().containsItem(DragonSlayer.WORMBRAIN_PIECE)) {
