...
public final class ImpJarCreatePlugin extends UseWithHandler {
	...
	public ImpJarCreatePlugin() {
		super(6097, 11264, 1939, 4525, 4535, 4544, 11262, 10012);
	}
	...
	public boolean handle(NodeUsageEvent event) {
		...
		if (event.getUsedWith() instanceof GameObject) {
			if ((event.getUsedItem().getId() >= 4525 && event.getUsedItem().getId() <= 4544) || event.getUsedItem().getId() == 1939 || event.getUsedItem().getId() == 11262 || event.getUsedItem().getId() == 10012) {
	...
	private void fillOilStill(Player player, Item used) {
		...
		if (used.getId() == 1939) {
			...
			if (configValue == 0) {
				...
				if (player.getInventory().contains(4525, 1) || player.getInventory().contains(4535, 1) || player.getInventory().contains(4544, 1)) {
					Item replace = new Item(used.getId() == 4525 ? 4522 : used.getId() == 4535 ? 4537 : 4546);
