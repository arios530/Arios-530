...
public final class MageArenaPlugin extends OptionHandler {
	...
	public void handlePool(final Player player, boolean enter, final Location dest, final GameObject object) {
		...
		if (enter) {
			...
			player.lock();
			AgilityHandler.walk(player, -1, start, middle, ForceMovement.WALK_ANIMATION, 0.0, null);
			GameWorld.submit(new Pulse(1, player) {
				int counter;
		...
		AgilityHandler.walk(player, -1, start, end, ForceMovement.WALK_ANIMATION, 0.0, "Your boots get wet.");
		GameWorld.submit(new Pulse(1, player) {
