...
public final class BarrowsPuzzle extends ComponentPlugin {
	...
	public static void open(Player player, int index) {
		...
		for (int i = 0; i < puzzle.questionModels.length; i++) {
			PacketRepository.send(DisplayModel.class, new DisplayModelContext(player, ModelType.MODEL, puzzle.questionModels[i], 0, 25, 1 + i));
		}
		for (int i = 0; i < puzzle.answerModels.length; i++) {
			PacketRepository.send(DisplayModel.class, new DisplayModelContext(player, ModelType.MODEL, puzzle.answerModels[i] & 0xFFFF, 0, 25, 6 + i));
		}
	}
	...
	public boolean handle(Player player, Component component, int opcode, int button, int slot, int itemId) {
		...
		case 8:
			...
			boolean correct = ((player.getAttribute("puzzle:answers", new int[3])[button - 6] >> 16) & 0xFF) == 1;
			if (!correct) {
