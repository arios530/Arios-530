...
public final class ModeratorCommand extends CommandPlugin {
	...
	public boolean parse(final Player player, String name, String[] args) {
		...
		case "kick":
			...
			} else if (WorldCommunicator.isEnabled()) {
		...
		case "players":
			...
			for (Player p : Repository.getPlayers()) {
				...
				if (p == null || (p.isAdmin() && !GameWorld.getSettings().isDevMode()) && !player.isAdmin()) {
					continue;
				}
				builder.append(red + "<img=" + (Rights.getChatIcon(p) - 1) + ">" + p.getUsername() + " [ip=" + p.getDetails().getIp() + ",  loc=" + p.getLocation() + "]<br><br>");
				count++;
			}
	...
	private Player punish(Player player, String target, String[] args, int type) {
		if (SystemManager.getSystemConfig().isAdmin(target)) {
	...
	private void checkBank(Player player, String[] args) {
		...
		Player o = Repository.getPlayerByDisplay(args[1]);
		if (o == null) {
			o = Repository.getPlayerFile(args[1]);
			if (o == null) {
		...
		PacketRepository.send(ContainerPacket.class, new ContainerContext(player, 12, 89, 90, new Item[] {}, 0, false));
		PacketRepository.send(ContainerPacket.class, new ContainerContext(player, 12, 89, 90, o.getBank().toArray(), false, slots));
