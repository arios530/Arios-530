...
public final class GDiplomacyCutscene extends CutscenePlugin {
	...
	public boolean start(final Player player, final boolean login, Object... args) {
		...
		final NPC grubfoot = NPC.create(quest.getStage() == 10 ? 4495 : quest.getStage() == 20 ? 4497 : quest.getStage() == 30 ? 4498 : 4496, getBase().transform(10, 55, 0));
		...
		public void sendGrubFoot(final GrubFoot grubFoot, final int endStage) {
			...
			GameWorld.submit(new Pulse(1, player) {
				...
				public boolean pulse() {
					switch (++counter) {
					case 1:
						player.getDialogueInterpreter().sendPlaneMessage(true, "Grubfoot goes into the changing room...");
						break;
					case 7:
					...
					case 11:
						...
						break;
					case 12:
						player.getDialogueInterpreter().sendPlaneMessage(true, "...And emerges wearing " + type.name().toLowerCase() + " armour.");
						break;
					case 13:
						Pathfinder.find(grubfoot, npc.getLocation().transform(-1, 0, 0)).walk(grubfoot);
						break;
		...
		private void handleFinishDialogues() {
			if (stage == 120) {
				if (player.getInventory().remove(type.getMail())) {
					quest.setStage(quest.getStage() + 10);
				}
				end();
				cutscene.unpause();
