...
/**
 * Represents the plugin used to handle node interactions related to dragon
 * slayer.
 * @author 'Vexia
 * @version 1.0
 */
public final class DragonSlayerPlugin extends OptionHandler {
	...
	public boolean handle(final Player player, Node node, String option) {
		...
		case 2661:
		case 2669:
			if (player.getQuestRepository().getQuest("Dragon Slayer").getStage() != 100) {
			...
			break;
		case 742:
			if (player.getQuestRepository().getQuest("Dragon Slayer").getStage() == 40 && (player.getInventory().containsItem(DragonSlayer.ELVARG_HEAD) || player.getInventory().containsItem(DragonSlayer.ELVARG_HEAD))) {
			...
			if (player.getQuestRepository().getQuest("Dragon Slayer").getStage() > 40) {
		...
		case 25161:
			...
			if (player.getQuestRepository().getQuest("Dragon Slayer").getStage() == 40 && !player.getInventory().containsItem(DragonSlayer.ELVARG_HEAD)) {
				ForceMovement movement = new ForceMovement(player, player.getLocation(), player.getLocation().transform(player.getLocation().getX() == 2845 ? 2 : -2, 0, 0), new Animation(839));
				movement.run(player, 10);
		...
		case 25213:
			...
			break;
		case 2606:
			if (player.getLocation().getY() < 9600 && !player.getSavedData().getQuestData().getDragonSlayerAttribute("memorized") && player.getQuestRepository().getQuest("Dragon Slayer").getStage() != 100) {
