...
public final class ShieldArravPlugin extends OptionHandler {
	...
	public boolean handle(Player player, Node node, String option) {
		...
		case 2401:
			switch (option) {
			case "search":
				if (quest.getStage() == 70 && ShieldofArrav.isBlackArm(player)) {
		...
		case 2404:
			...
			case "close":
				...
				break;
			case "search":
				if (quest.getStage() == 70 && ShieldofArrav.isPhoenix(player)) {
		...
		case 761:
			...
			break;
		case 2398:
			if (quest.getStage() == 60 && ShieldofArrav.isBlackArmMission(player) && !player.getInventory().containsItem(ShieldofArrav.KEY)) {
				...
				return true;
			} else if (quest.getStage() == 60 && player.getInventory().containsItem(ShieldofArrav.KEY)) {
				DoorActionHandler.handleAutowalkDoor(player, (GameObject) node);
				return true;
			}
			if (quest.getStage() == 70 && player.getInventory().containsItem(ShieldofArrav.KEY)) {
