...
public final class WaterfallPlugin extends OptionHandler {
	...
	public Plugin<Object> newInstance(Object arg) throws Throwable {
		...
		PluginManager.definePlugin(new AlmeraDialogue(), new BookOnBaxtorianPlugin(), new GolrieDialogue(), new HadleyDialogue(), new HudonDialogue(), new WaterfallTreeDialogue());
		return this;
	}
	...
	public boolean handle(final Player player, Node node, String option) {
		...
		case 1999:// Second key (in the actual waterfall dungeon)
			if (player.getInventory().contains(298, 1) || quest.getStage() < 30) {
				player.getPacketDispatch().sendMessage("You search the crate and find nothing.");
			} else if (quest.getStage() >= 30 && !player.getInventory().contains(298, 1)) {
		...
		case 2002://
			if (player.getLocation().equals(new Location(2568, 9893))) {
				...
				if (quest.getStage() >= 100 && node.getLocation().equals(new Location(2566, 9901)) && player.getLocation().equals(new Location(2566, 9901))) {
		...
		case 305:
			...
			break;
		case 1990:
			if (player.getInventory().contains(293, 1) || quest.getStage() < 30) {
				player.getPacketDispatch().sendMessage("You search the crate and find nothing.");
			} else if (quest.getStage() >= 30 && !player.getInventory().contains(293, 1)) {
				player.getPacketDispatch().sendMessage("You find a large key.");
		...
		case 1991:
			...
			break;
		case 1989:
			if (player.getInventory().contains(292, 1) || quest.getStage() != 20) {
				player.getPacketDispatch().sendMessage("You search the bookcase and find nothing of interest.");
				break;
			} else if (!player.hasItem(new Item(292)) && quest.getStage() == 20) {
		...
		case 1996:
			...
			player.lock(6);
			AgilityHandler.walk(player, -1, player.getLocation(), new Location(2512, 3471, 0), Animation.create(772), 0, null);
	...
	public void playRunestoneGraphics(final Player player) {
		player.getPacketDispatch().sendGlobalPositionGrapicc(580, new Location(2562, 9914));
		player.getPacketDispatch().sendGlobalPositionGrapicc(580, new Location(2562, 9912));
		player.getPacketDispatch().sendGlobalPositionGrapicc(580, new Location(2562, 9910));
		player.getPacketDispatch().sendGlobalPositionGrapicc(580, new Location(2569, 9914));
		player.getPacketDispatch().sendGlobalPositionGrapicc(580, new Location(2569, 9912));
		player.getPacketDispatch().sendGlobalPositionGrapicc(580, new Location(2569, 9910));
	}
	...
	public class WaterfallUseWithHandler extends UseWithHandler {
		...
		public boolean handle(NodeUsageEvent event) {
			...
			if (useditem.getId() == ROPE.getId() && object.getId() == 1996 || object.getId() == 1997) {
				...
				player.getPulseManager().run(new Pulse(2, player) {
					...
					public boolean pulse() {
						...
						AgilityHandler.walk(player, -1, player.getLocation(), new Location(2512, 3469, 0), Animation.create(776), 0, null);
			...
			if (useditem.getId() == AMULET.getId() && object.getId() == 2006 && quest.getStage() != 100 && !player.getLocation().equals(new Location(2603, 9914))) {
				if (player.getAttribute("waterfall_placed_runes") == null) {
			...
			if (useditem.getId() == URN.getId() && object.getId() == 2014 && quest.getStage() != 100) {
