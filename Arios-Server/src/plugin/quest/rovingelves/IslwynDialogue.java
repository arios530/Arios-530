...
public class IslwynDialogue extends DialoguePlugin {
	...
	public boolean handle(int interfaceId, int buttonId) {
		...
		case 0:
			if (quest.getStage() == 10 || quest.getStage() == 15) {
			...
			if (quest.getStage() == 20) {
				...
				stage = 19;
			} else if (quest.getStage() != 10 && quest.getStage() != 15) {
		...
		case 27:
			...
			break;
		case 30:
			if (quest.getStage() != 100 || quest.getState() != QuestState.COMPLETED) {
	...
	public boolean open(Object... args) {
		final Quest quest = player.getQuestRepository().getQuest(RovingElves.NAME);
		final Quest waterfall = player.getQuestRepository().getQuest(WaterFall.NAME);
		if (quest.getState() == QuestState.NOT_STARTED && waterfall.getState() == QuestState.COMPLETED) {
			interpreter.sendDialogues(player, FacialExpression.NORMAL, "Hello there.");
			stage = 0;
		}
		if (!waterfall.getState().equals(QuestState.COMPLETED)) {
			interpreter.sendDialogues(player, FacialExpression.NORMAL, "Hello there.");
			stage = 1000;
		}
		if (quest.getState().equals(QuestState.STARTED) && quest.getStage() >= 10) {
			interpreter.sendDialogues(player, FacialExpression.NORMAL, "Hello Islwyn.");
			stage = 0;
		}
		if (quest.getState().equals(QuestState.COMPLETED) || quest.getStage() >= 100) {
