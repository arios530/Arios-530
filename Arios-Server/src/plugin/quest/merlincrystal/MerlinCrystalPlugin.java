...
import org.arios.game.content.global.action.DropItemHandler;
...
public final class MerlinCrystalPlugin extends OptionHandler {
	...
	public Plugin<Object> newInstance(Object arg) throws Throwable {
		...
		ObjectDefinition.forId(60).getConfigurations().put("option:climb-up", this);
	...
	public boolean handle(Player player, Node node, String option) {
		...
		switch (id) {
		case 62:
			if (quest.getStage() == 90 && player.getEquipment().contains(35, 1)) {
		...
		case 72:
			...
			return true;
		case 530:
			if (player.getLocation().equals(new Location(2780, 3515, 0)) && quest.getStage() == 80) {
			...
			return true;
		case 60:
			if (player.getLocation().withinDistance(Location.create(3013, 3245, 0)) && quest.getStage() == 60 && player.getAttribute("beggar_npc") == null) {
	...
	public final class MerlinCrystalDialogue extends DialoguePlugin {
		...
		public boolean open(Object... args) {
			final Quest quest = player.getQuestRepository().getQuest(MerlinCrystal.NAME);
			if (quest.getStage() == 99) {
				npc = (NPC) args[0];
				npc("Thank you! Thank you! Thank you!");
				stage = 900;
				return true;
			}
			if (quest.getStage() == 40 && player.getLocation().withinDistance(new Location(2801, 3442, 0), 2)) {
				player.getDialogueInterpreter().sendDialogue("The crate is empty. It's just about big enough to hide inside.");
				stage = 0;
			}
			if (quest.getStage() == 50 && player.getLocation().withinDistance(new Location(2770, 3402, 2), 8)) {
	...
	public final class BeggarNPC extends NPC {
		...
		public boolean isHidden(final Player player) {
			if (player.getQuestRepository().getQuest(MerlinCrystal.NAME).getStage() == 60 && this.getAttribute("beggar_owner", "").equals(player.getUsername())) {
				return false;
			}
			return true;
		}
	...
	public final class ThrantaxNPC extends NPC {
		...
		@Override
		public boolean isHidden(final Player player) {
			if (player.getQuestRepository().getQuest(MerlinCrystal.NAME).getStage() == 80 && this.getAttribute("thrantax_owner", "").equals(player.getUsername())) {
				return false;
			}
			return true;
		}
