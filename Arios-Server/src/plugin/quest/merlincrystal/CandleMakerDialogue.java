...
public final class CandleMakerDialogue extends DialoguePlugin {
	...
	public void init() {
		...
		PluginManager.definePlugin(new OptionHandler() {
			...
			public boolean handle(Player player, Node node, String option) {
				NPC npc = node.asNpc();
				Quest quest = player.getQuestRepository().getQuest(MerlinCrystal.NAME);
	...
	public boolean handle(int interfaceId, int buttonId) {
		...
		switch (stage) {
		case 2:
			if (quest.getStage() == 50 || quest.getStage() == 60) {// the
				// player
				// has
				// defeated
				// mordred
				...
				stage = 3;
			} else if (quest.getStage() > 60) {// they're farther along in
				// the quest
				interpreter.sendOptions("Select an Option", "Have any black candles?", "Yes, let me see your stock.", "No thanks.");
				stage = 25;
		...
		case 30:
			...
			break;
		case 40:
			if (quest.getStage() == 50 && player.getInventory().contains(MerlinCrystalPlugin.BUCKET_OF_WAX.getId(), 1)) {
