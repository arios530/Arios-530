...
public final class AnimalMagnetismPlugin extends OptionHandler {
	...
	public boolean handle(Player player, Node node, String option) {
		...
		case 5198:
		case 5199:
			if (player.getQuestRepository().getQuest(AnimalMagnetism.NAME).getStage() == 0) {
		...
		case 4252:
			...
			break;
		case 10492:
			ResearchNoteHandler.open(player);
			break;
		case 688:
			player.lock(1);
	...
	public static final class UndeadTreePlugin extends UseWithHandler {
		 ...
		 * The axe ids.
		 */
		private static final int[] IDS = new int[] { 1355, 1357, 1359, 6739 };
			...
			PluginManager.definePlugin(new OptionHandler() {
				...
				public boolean handle(Player player, Node node, String option) {
					...
					if (quest.getStage() <= 28) {
						...
						player.sendMessage("The axe bounces off the undead wood." + (quest.getStage() == 28 || quest.getStage() == 29 ? " I should report this to Ava." : ""));
						return true;
					}
	...
	public static final class ResearchNoteHandler extends ComponentPlugin {
		 ...
		 * The button ids.
		 */
		private static final int[][] BUTTONS = new int[][] { { 24, 23, 12 }, { 27, 25, 8 }, { 30, 29, 14 }, { 33, 32, 16 }, { 36, 35, 6 }, { 39, 38, 18 }, { 42, 41, 20 }, { 45, 44, 22 }, { 48, 47, 10 } };
		 ...
		 * @param player the player.
		 */
		private static void clearCache(Player player) {
