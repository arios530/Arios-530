...
public final class TouristTrapPlugin extends OptionHandler {
	...
	public boolean handle(final Player player, Node node, String option) {
		...
		case "open":
			...
			case 2677:
				...
				if (quest.getStage() <= 54 && quest.getStage() != 53) {
				...
				} else if (quest.getStage() == 53) {
					if (!quest.hasItem(TouristTrap.TECHNICAL_PLANS)) {
			...
			case 2674:
			case 2673:
				if (quest.getStage() > 60 && quest.getStage() < 98 && player.getInventory().containsItem(TouristTrap.ANNA_BARREL)) {
					player.lock();
		...
		case "search":
			...
			case 18902:
				if (!hasItem(player, TouristTrap.CELL_DOOR_KEY)) {
					...
					break;
				} else if (!hasItem(player, TouristTrap.WROUGHT_IRON_KEY) && player.getQuestRepository().isComplete(quest)) {
	...
	public static final class WinchDialogue extends DialoguePlugin {
		...
		public boolean handle(int interfaceId, int buttonId) {
			...
			case 516:
				...
				player.getQuestRepository().getQuest(TouristTrap.NAME).setStage(71);
				end();
				break;
			case 0:
				...
				GameWorld.submit(new Pulse(1, player) {
					...
					public boolean pulse() {
						...
						case 22:
							...
							break;
						case 33:
							player.getQuestRepository().getQuest(TouristTrap.NAME).setStage(70);
							player.getInventory().remove(TouristTrap.ANNA_BARREL);
			...
			case 2:
				...
				GameWorld.submit(new Pulse(1, player) {
					...
					public boolean pulse() {
						...
						case 6:
							player.unlock();
							player.getQuestRepository().getQuest(TouristTrap.NAME).setStage(95);
	...
	public static final class BarrelDialogue extends DialoguePlugin {
		...
		public boolean open(Object... args) {
			...
			if ((quest.getStage() == 70 || quest.getStage() == 72) && !quest.hasItem(TouristTrap.ANNA_BARREL)) {
	...
	public static final class BedabinAnvilHandler extends UseWithHandler {
		...
		public boolean handle(NodeUsageEvent event) {
			final Player player = event.getPlayer();
			final Quest quest = player.getQuestRepository().getQuest(TouristTrap.NAME);
			if (quest.getStage() == 54 && player.getInventory().containsItem(TouristTrap.TECHNICAL_PLANS)) {
		...
		public static final class BedabinAnvilDialogue extends DialoguePlugin {
			...
			public static final class ProtoTypePulse extends SkillPulse<GameObject> {
				...
				public boolean reward() {
					...
					if (ticks == 4) {
						player.getDialogueInterpreter().sendPlaneMessage(true, "You begin experimenting in forging the weapon...");
					} else if (ticks == 8) {
