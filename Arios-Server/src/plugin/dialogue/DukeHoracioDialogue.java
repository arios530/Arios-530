...
public final class DukeHoracioDialogue extends DialoguePlugin {
	...
	public boolean handle(int interfaceId, int buttonId) {
		if (player.getQuestRepository().getQuest("Dragon Slayer").getStage() == 20 && !player.getInventory().containsItem(DragonSlayer.SHIELD) && !player.getBank().containsItem(DragonSlayer.SHIELD) && !player.getEquipment().containsItem(DragonSlayer.SHIELD)) {
			switch (stage) {
			case 0:
		...
		switch (stage) {
		case 0:
			if (player.getQuestRepository().getQuest("Dragon Slayer").getStage() == 100 && !player.getInventory().containsItem(DragonSlayer.SHIELD)) {
		...
		case 100:
			...
			break;
		case 101:
			interpreter.sendDialogue("The Duke hands you an " + quest.getBlue() + "air talisman</col>.");
			stage = 102;
			break;
		case 102:
