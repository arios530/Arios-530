...
public final class KaqemeexDialogue extends DialoguePlugin {
	...
	public boolean handle(int interfaceId, int buttonId) {
		...
		case 0:
			...
			if (player.getQuestRepository().getQuest("Drudic Ritual").getStage() == 10) {
				interpreter.sendDialogues(npc, FacialExpression.NORMAL, "Hello again.");
				stage = 40;
				break;
			}
			if (player.getQuestRepository().getQuest("Drudic Ritual").getStage() == 99) {
			...
			break;
		case 1:
			if (player.getQuestRepository().getQuest("Drudic Ritual").getState() != QuestState.NOT_STARTED) {
		...
		case 25:
			...
			break;
		case 26:
			player.getQuestRepository().getQuest("Drudic Ritual").setStage(10);
			player.getQuestRepository().getQuest("Drudic Ritual").setState(QuestState.STARTED);
		...
		case 200:
			...
			player.getQuestRepository().getQuest("Drudic Ritual").finish();
			break;
		case 500:
			switch (buttonId) {
			case 1:
