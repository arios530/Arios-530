...
public final class FatherUhrneyDialogue extends DialoguePlugin {
	...
	public boolean handle(int interfaceId, int buttonId) {
		switch (stage) {
		case 0:
			if (player.getQuestRepository().getQuest("The Restless Ghost").getStage() == 0) {
				interpreter.sendOptions("Select an Option", "Well, that's friendly.", "I've come to respossess your house.");
				stage = 1;
			} else if (player.getQuestRepository().getQuest("The Restless Ghost").getStage() == 10) {
		...
		case 508:
			...
			player.getQuestRepository().getQuest("The Restless Ghost").setStage(20);
