...
public final class DoricDialogue extends DialoguePlugin {
	...
	public boolean handle(int interfaceId, int buttonId) {
		...
		case 15:
			...
			break;
		case 16:
			player.getQuestRepository().getQuest("Doric's Quest").setStage(1);
			player.getQuestRepository().getQuest("Doric's Quest").setState(QuestState.STARTED);
		...
		case 202:
			if (player.getInventory().remove(REQUIREMENTS)) {
				...
				player.getQuestRepository().getQuest("Doric's Quest").finish();
			}
			break;
		case 500:
