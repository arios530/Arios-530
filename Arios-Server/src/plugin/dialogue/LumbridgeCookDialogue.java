...
public final class LumbridgeCookDialogue extends DialoguePlugin {
	...
	public boolean open(Object... args) {
		npc = (NPC) args[0];
		if (player.getQuestRepository().getQuest("Cook's Assistant").getStage() == 10) {
		...
		if (player.getQuestRepository().getQuest("Cook's Assistant").getStage() == 100) {
	...
	public boolean handle(int interfaceId, int buttonId) {
		switch (stage) {
		case 0:
			if (player.getQuestRepository().getQuest("Cook's Assistant").getStage() == 0) {
		...
		case 15:
			...
			break;
		case 678:
			player.getQuestRepository().getQuest("Cook's Assistant").setStage(10);
			player.getQuestRepository().getQuest("Cook's Assistant").setState(QuestState.STARTED);
		...
		case 960:
			...
			player.getQuestRepository().getQuest("Cook's Assistant").finish();
			break;
		case 768:
