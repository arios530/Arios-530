...
public final class LumbridgeGuideDialogue extends DialoguePlugin {
	...
	public boolean handle(int interfaceId, int buttonId) {
		switch (stage) {
		case 0:
			if (!player.isDonator() && !player.isStaff() && !player.getIronmanManager().isIronman() || (player.getIronmanManager().isPermanent() && !player.isDonator())) {
			...
			if (player.getIronmanManager().isIronman() && !player.getIronmanManager().isPermanent()) {
