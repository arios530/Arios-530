...
public final class FatherAereckDialogue extends DialoguePlugin {
	...
	public boolean open(Object... args) {
		npc = (NPC) args[0];
		if (player.getQuestRepository().getQuest("The Restless Ghost").getStage() == 10) {
			interpreter.sendDialogues(npc, FacialExpression.NORMAL, "Have you got rid of the ghost yet?");
			stage = 520;
			return true;
		}
		if (player.getQuestRepository().getQuest("The Restless Ghost").getStage() == 20) {
		...
		if (player.getQuestRepository().getQuest("The Restless Ghost").getStage() == 30) {
		...
		if (player.getQuestRepository().getQuest("The Restless Ghost").getStage() == 40) {
	...
	public boolean handle(int interfaceId, int buttonId) {
		...
		case 509:
			...
			break;
		case 510:
			player.getQuestRepository().getQuest("The Restless Ghost").setStage(10);
			player.getQuestRepository().getQuest("The Restless Ghost").setState(QuestState.STARTED);
		...
		case 10:
			...
			BitregisterAssembler.send(player, 652, 34, 0, 13, 1);
