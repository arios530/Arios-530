...
public final class ApothecaryDialogue extends DialoguePlugin {
	...
	public boolean open(Object... args) {
		npc = (NPC) args[0];
		if (player.getQuestRepository().getQuest("Romeo & Juliet").getStage() == 40) {
			interpreter.sendDialogues(player, null, "Apothecary. Father Lawrence sent me.");
			stage = 500;
			return true;
		}
		if (player.getQuestRepository().getQuest("Romeo & Juliet").getStage() == 50) {
		...
		if (player.getQuestRepository().getQuest("Romeo & Juliet").getStage() == 60) {
	...
	public boolean handle(int interfaceId, int buttonId) {
		...
		case 505:
			...
			break;
		case 506:
			player.getQuestRepository().getQuest("Romeo & Juliet").setStage(50);
		...
		case 639:
			...
			break;
		case 640:
			player.getQuestRepository().getQuest("Romeo & Juliet").setStage(60);
			end();
			break;
		}
