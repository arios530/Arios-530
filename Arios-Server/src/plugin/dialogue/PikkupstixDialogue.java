	...
	public boolean handle(int interfaceId, int buttonId) {
		...
		case 100:
			...
			case 604:
				...
				break;
			case 9000:
				if (player.getDetails().getShop().hasPerk(Perks.SHARD_BARGAINER)) {
					...
					interpreter.sendDialogue("You need the Shard Bargainer perk to buy shards in bulk.", "The perk is available to all players.");
					stage = 604;
				}
				break;
			case 9001:
			...
			case 9004:
				...
				if (player.getInventory().getAmount(995) < price) {
					player("I don't have enough coins with me.", "I'll be back when I do.");
					stage = 604;
					return true;
				}
				if (player.getInventory().getAmount(995) >= price && player.getInventory().freeSlots() >= 1) {
