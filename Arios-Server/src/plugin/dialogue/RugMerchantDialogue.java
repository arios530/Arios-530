...
public final class RugMerchantDialogue extends DialoguePlugin {
	 ...
	 */
	private static final Animation FLOATING_ANIMATION = new Animation(2261);
	...
	private void sendOptions(RugDestination[] destinations) {
		...
		if (destinations.length == 1) {
			npc("Ttravel back to " + destinations[0].getName() + "?");
			stage = 8;
			return;
		}
				...
				public boolean pulse() {
					...
					default:
						...
						if (index == 0 || player.getLocation().equals(locs[index - 1])) {
							AgilityHandler.forceWalk(player, -1, player.getLocation(), locs[index++], new Animation(2261), 40, 0.0, null);
						}
						return false;
					}
					return false;
