...
/**
 * Represents the dialogue to handle the transcript of nd.
 * @author 'Vexia
 * @date 31/12/2013
 */
public final class NedDialogue extends DialoguePlugin {
	...
	public boolean open(Object... args) {
		...
		if (player.getQuestRepository().getQuest("Dragon Slayer").getState() == QuestState.STARTED && player.getQuestRepository().getQuest("Prince Ali Rescue").getState() == QuestState.STARTED) {
		...
		if (player.getQuestRepository().getQuest("Dragon Slayer").getStage() == 30) {
			player("Will you take me to Crandor now, then?");
			stage = 520;
			return true;
		}
	...
	public boolean handle(int interfaceId, int buttonId) {
		...
		if ((q == null || q.equals("dSlayer")) && (player.getQuestRepository().getQuest("Dragon Slayer").getStage() == 20 || (player.getQuestRepository().getQuest("Dragon Slayer").getStage() == 30) && player.getSavedData().getQuestData().getDragonSlayerAttribute("repaired"))) {
			switch (stage) {
			case 499:
			...
			case 515:
				if (player.getInventory().remove(DragonSlayer.CRANDOR_MAP)) {
					...
					player.getQuestRepository().getQuest("Dragon Slayer").setStage(30);
					stage = 516;
				}
				break;
			case 516:
		...
		switch (stage) {
		case 0:
			interpreter.sendOptions("Select an Option", "Yes, I would like some rope.", "No thanks Ned, I don't need any.");
			stage = 1;
			break;
		case 1:
			switch (buttonId) {
