...
public class KingRoaldDialogue extends DialoguePlugin {
	...
	public boolean open(Object... args) {
		npc = (NPC) args[0];
		switch (player.getQuestRepository().getQuest("Priest in Peril").getStage()) {
		case 0:
	...
	public boolean handle(int interfaceId, int buttonId) {
		if (player.getQuestRepository().getQuest("Shield of Arrav").getStage() == 70 && player.getInventory().containsItem(ShieldofArrav.PHOENIX_SHIELD) || player.getInventory().containsItem(ShieldofArrav.BLACKARM_SHIELD)) {
			switch (stage) {
			case 0:
		...
		if (player.getQuestRepository().getQuest("Shield of Arrav").getStage() == 70 && player.getInventory().containsItem(ShieldofArrav.BLACKARM_CERTIFICATE) || player.getInventory().containsItem(ShieldofArrav.PHOENIX_CERTIFICATE)) {
			switch (stage) {
			case 0:
		...
		if (player.getQuestRepository().getQuest("Shield of Arrav").getStage() == 70 && player.getInventory().containsItem(CERTIFICATE)) {
			switch (stage) {
			case 0:
				player("Your majesty, I have come to claim the reward for the", "return of the Shield of Arrav.");
				stage = 1;
			...
			case 6:
				if (player.getInventory().remove(CERTIFICATE)) {
					...
					player.getQuestRepository().getQuest("Shield of Arrav").finish();
					end();
				}
				break;
			}
		...
		case 1:
			...
			if (quest.getStage() == 12) {
				interpreter.sendDialogues(npc, FacialExpression.NORMAL, "You have news of Drezel for me?");
				stage = 23;
			}
			if (quest.getStage() == 13) {
