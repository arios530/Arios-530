...
public final class DrHarlowDialogue extends DialoguePlugin {
	...
	public boolean handle(int interfaceId, int buttonId) {
		switch (stage) {
		case 0:
			if (player.getQuestRepository().getQuest("Vampire Slayer").getStage() == 10) {
				interpreter.sendOptions("Select an Option", "No, you've had enough.", "Morgan needs your help!");
				stage = 1;
			} else if (player.getQuestRepository().getQuest("Vampire Slayer").getStage() == 20) {
			...
			} else if (player.getQuestRepository().getQuest("Vampire Slayer").getStage() == 30) {
		...
		case 9:
			...
			break;
		case 10:
			player.getQuestRepository().getQuest("Vampire Slayer").setStage(20);
			end();
			break;
		case 2:
			end();
		...
		case 20:
			if (player.getInventory().remove(ITEMS[1])) {
				...
				player.getQuestRepository().getQuest("Vampire Slayer").setStage(30);
				stage = 21;
			}
			break;
		case 21:
