...
public final class LostFoundRandomEvent extends AntiMacroEvent {
	...
	public LostFoundRandomEvent(Player player) {
		super("Lost and found", false, true, -1);
		super.player = player;
	}
	...
	public boolean enter(Entity entity) {
		if (entity instanceof Player) {
			...
			player.getDialogueInterpreter().sendPlaneMessage(false, "There has been a fault in the teleportation matrix. Please operate the", "odd appendage out to be forwarded to your destination.");
		}
				...
				if (isOddAppendage(player, object)) {
					GameWorld.submit(new Pulse(4) {
						...
						public boolean pulse() {
							player.getDialogueInterpreter().sendPlaneMessage(false, "The Abyssal Services Department apologises for the inconvenience.");
							return true;
						}
					});
					player.getInventory().add(player.getAttribute("teleport:items", new Item[0]));
					player.getTeleporter().send(player.getAttribute("l&f_dest", ServerConstants.getHomeLocation()), TeleportManager.getType(player), -1);
				} else {
					setRandomAppendage(player);
